...
class PurchaseFlightTicket(SequentialTaskSet): # класс с задачами (содержит основной сценарий)
    ...
    def uc03_FindFlight_InputParams(self):
        ...
        with self.client.post(
            ...
            self.outboundFight = re.search(r"<input type=\"radio\" name=\"outboundFlight\" value=\"(.*)\">",r_03_0response.text).group(1)
    ...
    def uc04_ChooseFlightOption(self):
        ...
        r04_00_body = f"outboundFlight={quote_plus(self.outboundFight)}&numPassengers=1&advanceDiscount=0&seatType={self.seat_type}&seatPref={self.seat_pref}&reserveFlights.x=60&reserveFlights.y=2"
    ...
    def uc05_ConfirmFlightBooking(self):
        ...
        r05_00_body = f"firstName={self.firstname}&lastName={self.lastname}&address1={quote_plus(self.address1)}&address2={quote_plus(self.address2)}&pass1={quote_plus(self.firstname+ ' ' +self.lastname)}&creditCard={generateRandomCardNumber()}&expDate={quote_plus(self.expDate)}&numPassengers=1&seatType={self.seat_type}&seatPref={self.seat_pref}&outboundFlight={quote_plus(self.outboundFight)}&advanceDiscount=0&returnFlight=&JSFormSubmit=off&buyFlights.x=68&buyFlights.y=16&.cgifields=saveCC"
